...
namespace AwithGameFrame.DataPersistence.Examples
{
    /// <summary>
    /// 数据持久化使用示例
    /// 展示如何使用不同的存储方式
    ...
    public class DataPersistenceUsageExample : MonoBehaviour
    {
        ...
        async void Start()
        {
            ...
            var initResult = await DataPersistence.InitializeAsync(config);
        ...
        private async UniTask TestUserSettings()
        {
            ...
            // 保存设置
            var saveResult = await DataPersistence.SaveSettingsAsync("game_settings", settings);
            Debug.Log($"保存用户设置结果: {saveResult}");

            // 加载设置
            var (loadedSettings, loadResult) = await DataPersistence.LoadSettingsAsync<GameSettings>("game_settings");
        ...
        private async UniTask TestGameData()
        {
            ...
            // 保存游戏数据
            var saveResult = await DataPersistence.SaveGameDataAsync("player_data", playerData);
            Debug.Log($"保存游戏数据结果: {saveResult}");

            // 加载游戏数据
            var (loadedData, loadResult) = await DataPersistence.LoadGameDataAsync<PlayerData>("player_data");
            if (loadResult == DataOperationResult.Success)
            {
        ...
        private async UniTask TestCacheData()
        {
            ...
            // 保存缓存数据
            var saveResult = await DataPersistence.SaveCacheDataAsync("cache_data", cacheData);
            Debug.Log($"保存缓存数据结果: {saveResult}");

            // 加载缓存数据
            var (loadedData, loadResult) = await DataPersistence.LoadCacheDataAsync<object>("cache_data");
            if (loadResult == DataOperationResult.Success)
            {
        ...
        private async UniTask TestComplexData()
        {
            ...
            // 保存复杂数据（使用SQLite）
            var saveResult = await DataPersistence.SaveComplexDataAsync("complex_player_data", complexData, DatabaseType.SQLite);
            Debug.Log($"保存复杂数据结果: {saveResult}");

            // 加载复杂数据
            var (loadedData, loadResult) = await DataPersistence.LoadComplexDataAsync<PlayerData>("complex_player_data", DatabaseType.SQLite);
            if (loadResult == DataOperationResult.Success)
            {
        ...
        private async UniTask TestManagementFeatures()
        {
            ...
            // 获取所有数据键
            var allKeys = await DataPersistence.GetAllKeysAsync();
            Debug.Log($"所有数据键: {string.Join(", ", allKeys)}");

            // 获取统计信息
            var stats = await DataPersistence.GetStatisticsAsync();
            Debug.Log($"存储统计信息: 总键数={stats.ItemCount}, 总大小={stats.TotalSize} bytes");

            // 检查数据是否存在
            var exists = await DataPersistence.ExistsAsync("player_data");
            Debug.Log($"player_data是否存在: {exists}");

            // 删除特定数据
            var deleteResult = await DataPersistence.DeleteAsync("cache_data");
            Debug.Log($"删除cache_data结果: {deleteResult}");

            // 验证删除
            var existsAfterDelete = await DataPersistence.ExistsAsync("cache_data");
