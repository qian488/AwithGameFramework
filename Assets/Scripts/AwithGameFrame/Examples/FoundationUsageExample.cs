...
namespace AwithGameFrame.Examples
{
    ...
    public class FoundationUsageExample : MonoBehaviour
    {
        ...
        private async UniTask TestDataPersistence()
        {
            ...
            // 初始化数据持久化系统
            var initResult = await AwithGameFrame.DataPersistence.DataPersistence.InitializeAsync();
            ...
            if (initResult == DataOperationResult.Success)
            {
                ...
                // 保存数据
                var saveResult = await AwithGameFrame.DataPersistence.DataPersistence.SaveAsync("player_data", playerData);
                Debug.Log($"保存数据结果: {saveResult}");

                // 加载数据
                var (loadedData, loadResult) = await AwithGameFrame.DataPersistence.DataPersistence.LoadAsync<PlayerData>("player_data");
