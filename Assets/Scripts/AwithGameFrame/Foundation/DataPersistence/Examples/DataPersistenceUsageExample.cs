...
namespace AwithGameFrame.Foundation.DataPersistence
{
    ...
    public class DataPersistenceUsageExample : MonoBehaviour
    {
        ...
        async void Start()
        {
            LoggingAPI.Info(LogCategory.DataPersistence, "=== 数据持久化使用示例开始 ===");
            ...
            LoggingAPI.Info(LogCategory.DataPersistence, $"数据持久化系统初始化结果: {initResult}");
            ...
            LoggingAPI.Info(LogCategory.DataPersistence, "=== 数据持久化使用示例完成 ===");
        }
        ...
        {
            LoggingAPI.Info(LogCategory.DataPersistence, "\n--- 测试用户设置存储（PlayerPrefs） ---");
            ...
            LoggingAPI.Info(LogCategory.DataPersistence, $"保存用户设置结果: {saveResult}");
            ...
            {
                LoggingAPI.Info(LogCategory.DataPersistence, $"加载用户设置成功: 音量={loadedSettings.masterVolume}, 全屏={loadedSettings.fullscreen}");
            }
        }
        ...
        private async UniTask TestGameData()
        {
            ...
            LoggingAPI.Info(LogCategory.DataPersistence, $"保存游戏数据结果: {saveResult}");

            // 加载游戏数据
            var (loadedData, loadResult) = await DataPersistenceAPI.LoadGameDataAsync<PlayerData>("player_data");
            if (loadResult == DataOperationResult.Success)
            {
                LoggingAPI.Info(LogCategory.DataPersistence, $"加载游戏数据成功: 玩家={loadedData.playerName}, 等级={loadedData.level}, 位置={loadedData.position}");
            }
        }
        ...
        private async UniTask TestCacheData()
        {
            ...
            LoggingAPI.Info(LogCategory.DataPersistence, $"保存缓存数据结果: {saveResult}");

            // 加载缓存数据
            var (loadedData, loadResult) = await DataPersistenceAPI.LoadCacheDataAsync<object>("cache_data");
            if (loadResult == DataOperationResult.Success)
            {
                LoggingAPI.Info(LogCategory.DataPersistence, $"加载缓存数据成功: {loadedData}");
            }
        }
        ...
        private async UniTask TestComplexData()
        {
            ...
            LoggingAPI.Info(LogCategory.DataPersistence, $"保存复杂数据结果: {saveResult}");
            ...
            if (loadResult == DataOperationResult.Success)
            {
                LoggingAPI.Info(LogCategory.DataPersistence, $"加载复杂数据成功: 玩家={loadedData.playerName}, 等级={loadedData.level}");
            }
        }
        ...
        private async UniTask TestManagementFeatures()
        {
            ...
            LoggingAPI.Info(LogCategory.DataPersistence, $"所有数据键: {string.Join(", ", allKeys)}");

            // 获取统计信息
            var stats = await DataPersistenceAPI.GetStatisticsAsync();
            LoggingAPI.Info(LogCategory.DataPersistence, $"存储统计信息: 总键数={stats.ItemCount}, 总大小={stats.TotalSize} bytes");

            // 检查数据是否存在
            var exists = await DataPersistenceAPI.ExistsAsync("player_data");
            LoggingAPI.Info(LogCategory.DataPersistence, $"player_data是否存在: {exists}");

            // 删除特定数据
            var deleteResult = await DataPersistenceAPI.DeleteAsync("cache_data");
            LoggingAPI.Info(LogCategory.DataPersistence, $"删除cache_data结果: {deleteResult}");

            // 验证删除
            var existsAfterDelete = await DataPersistenceAPI.ExistsAsync("cache_data");
            LoggingAPI.Info(LogCategory.DataPersistence, $"删除后cache_data是否存在: {existsAfterDelete}");
        }
    }
}
