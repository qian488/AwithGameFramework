...
namespace AwithGameFrame.Foundation.Examples
{
    ...
    public class FoundationUsageExample : MonoBehaviour
    {
        ...
        async void Start()
        {
            LoggingAPI.Info(LogCategory.Core, "=== Foundation包使用示例开始 ===");
            ...
            LoggingAPI.Info(LogCategory.Core, "=== Foundation包使用示例完成 ===");
        }

        private async UniTask TestAsyncOperations()
        {
            LoggingAPI.Info(LogCategory.Core, "\n--- 测试异步操作 ---");

            // 创建已完成的任务
            var result = await FoundationAPI.FromResult("Hello Foundation!");
            LoggingAPI.Info(LogCategory.Core, $"异步结果: {result}");

            // 延迟操作
            LoggingAPI.Info(LogCategory.Core, "开始延迟2秒...");
            await FoundationAPI.Delay(2000);
            LoggingAPI.Info(LogCategory.Core, "延迟完成!");
            ...
            LoggingAPI.Info(LogCategory.Core, "开始等待多个任务...");
            await FoundationAPI.WhenAll(task1, task2, task3);
            LoggingAPI.Info(LogCategory.Core, "所有任务完成!");
        }
        ...
        private async UniTask TestAnimationOperations()
        {
            ...
            // 移动动画
            LoggingAPI.Info(LogCategory.Core, "开始移动动画...");
            await FoundationAPI.MoveTo(transform, Vector3.up * 2, 1.0f);
            LoggingAPI.Info(LogCategory.Core, "移动动画完成!");

            // 缩放动画
            LoggingAPI.Info(LogCategory.Core, "开始缩放动画...");
            await FoundationAPI.ScaleTo(transform, Vector3.one * 1.5f, 1.0f);
            LoggingAPI.Info(LogCategory.Core, "缩放动画完成!");

            // 旋转动画
            LoggingAPI.Info(LogCategory.Core, "开始旋转动画...");
            await FoundationAPI.RotateTo(transform, Vector3.up * 360, 2.0f);
            LoggingAPI.Info(LogCategory.Core, "旋转动画完成!");
        ...
        private async UniTask TestSerializationOperations()
        {
            ...
            // 序列化
            LoggingAPI.Info(LogCategory.Core, "开始序列化...");
            var json = await FoundationAPI.SerializeAsync(playerData);
            LoggingAPI.Info(LogCategory.Core, $"序列化结果: {json}");

            // 验证JSON
            bool isValid = FoundationAPI.IsValidJson(json);
            LoggingAPI.Info(LogCategory.Core, $"JSON是否有效: {isValid}");

            // 反序列化
            LoggingAPI.Info(LogCategory.Core, "开始反序列化...");
            var deserializedData = await FoundationAPI.DeserializeAsync<PlayerData>(json);
            LoggingAPI.Info(LogCategory.Core, $"反序列化结果: 姓名={deserializedData.name}, 等级={deserializedData.level}, 经验={deserializedData.experience}");
        }
        ...
        private async UniTask TestDataPersistence()
        {
            ...
            LoggingAPI.Info(LogCategory.DataPersistence, $"数据持久化初始化结果: {initResult}");
            ...
            if (initResult == DataOperationResult.Success)
            {
                ...
                LoggingAPI.Info(LogCategory.DataPersistence, $"保存数据结果: {saveResult}");
                ...
                {
                    LoggingAPI.Info(LogCategory.DataPersistence, $"加载数据成功: 姓名={loadedData.name}, 等级={loadedData.level}, 经验={loadedData.experience}");
                }
                else
                {
                    LoggingAPI.Error(LogCategory.DataPersistence, $"加载数据失败: {loadResult}");
                }
            }
        }
    }
